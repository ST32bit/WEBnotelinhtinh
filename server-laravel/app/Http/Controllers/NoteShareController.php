...
class NoteShareController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        // Đồng bộ hóa nhãn dán giữa người chia sẻ (cha) và người nhận chia sẻ mới (con)
        \App\Http\Controllers\NoteController::syncSharedLabels($note);
    ...
    public function updateRole(Request $request): JsonResponse
    {
        ...
        $query = SharedNote::where('owner_id', $request->user()->id);

        if ($request->filled('share_id')) {
            $query->where('id', $request->share_id);
        } elseif ($request->filled('note_id') && $request->filled('email')) {
            $target = User::where('email', $request->email)->first();
            if (!$target) {
                return response()->json(['error' => 'Không tìm thấy người dùng'], 404);
            }
            $query->where('note_id', $request->note_id)->where('shared_with', $target->id);
        } else {
            return response()->json(['error' => 'Thiếu thông tin xác định lượt chia sẻ'], 400);
        }

        $share = $query->first();
        if (!$share) {
            return response()->json(['error' => 'Không tìm thấy lượt chia sẻ hoặc bạn không có quyền'], 403);
        }

        $share->update(['role' => $request->role]);

        return response()->json(['success' => true, 'message' => 'Đã cập nhật quyền thành công']);
    }
    ...
    public function destroy(Request $request): JsonResponse
    {
        $request->validate([
            'note_id'     => 'required|integer',
            'shared_with' => 'nullable|integer',
            'email'       => 'nullable|email',
        ]);

        $query = SharedNote::where('note_id', $request->note_id)
        ...
        if ($request->filled('shared_with')) {
            ...
            $target = User::where('email', $request->email)->first();
            if ($target) {
                $query->where('shared_with', $target->id);
            } else {
                return response()->json(['error' => 'Không tìm thấy người dùng'], 404);
            }
        } else {
            return response()->json(['error' => 'Thiếu thông tin người nhận'], 400);
        }
    ...
    public function publicJoin(Request $request, int $noteId): JsonResponse
    {
        ...
        if (!in_array($note->visibility, ['public', 'link'])) {
    ...
    /**
     * Tiêu chí 26: Poll for new or updated shared notes
     */
    public function poll(Request $request): JsonResponse
    {
        ...
        $changedShares = SharedNote::with(['note', 'owner'])
            ->where('shared_with', $user->id)
        ...
        return response()->json([
            'success' => true,
            'changed_shared' => $changedShares->values(),
            'total_count' => SharedNote::where('shared_with', $user->id)->count(),
            'server_time' => now()->toDateTimeString(),
        ]);
    }

    /**
     * Kiểm tra tính hợp lệ của email chia sẻ trước khi lưu
     */
    public function validateShare(Request $request): JsonResponse
    {
        $request->validate([
            'note_id' => 'required|integer',
            'email'   => 'required|email',
        ...
        // 1. Không thể chia sẻ với chính mình
        if ($email === strtolower($user->email)) {
            return response()->json(['error' => 'Không thể chia sẻ ghi chú cho chính bản thân mình'], 400);
        }

        // 2. Người dùng được chia sẻ phải tồn tại và kích hoạt
        $target = User::where('email', $email)->where('is_active', true)->first();
        if (!$target) {
            return response()->json(['error' => 'Không tìm thấy người dùng đã kích hoạt với email này'], 404);
        }

        // 3. Người dùng đã được chia sẻ note này rồi
        $exists = SharedNote::where('note_id', $request->note_id)
            ->where('shared_with', $target->id)
            ->exists();
        if ($exists) {
            return response()->json(['error' => 'Ghi chú này đã được chia sẻ với email này rồi'], 400);
        }
